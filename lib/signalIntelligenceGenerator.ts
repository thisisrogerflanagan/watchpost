...
export function generateSignalEnrichment(filing: FilingRecord): SignalEnrichment {
  ...
  const contacts = [
    ...
    {
      name: 'General Counsel / Chief Legal Officer',
...
Given the 4-day SEC reporting window and regulatory compliance mandates, our Incident Response & Forensics practice specializes in rapid containment verification, root-cause forensic analysis, and board-level compliance reporting.

Would you be open to a brief 15-minute executive briefing today to review our emergency IR retainer protocols?

Best regards,
[Partner Name] | Managing Director, Cybersecurity Practice`;

  const vendorPitchScript = `Hi [FirstName],

Congratulations on the recent leadership transition at ${companyName}. 

When security leadership changes, enterprise teams typically evaluate their security operations stack within the initial 90-day onboarding window. We help engineering teams streamline threat visibility and automate SEC regulatory compliance.

Do you have 10 minutes next Tuesday for a brief intro call?

Best regards,
[Your Name] | Enterprise Practice Lead`;

  const execBriefScript = `EXECUTIVE BRIEFING: $${ticker} (${companyName})
Filing Type: ${filing.item_105_flag ? 'SEC Item 1.05 Material Cybersecurity Incident' : 'SEC Item 5.02 Executive Transition'}
Materiality Impact Score: ${impactScore}/100 (${urgencyLevel} URGENCY)
Filing Date: ${new Date(filing.filing_date).toUTCString()}

MATERIAL SUMMARY:
${executiveSummary}

STRATEGIC ACTION ITEMS:
1. Initiate direct outreach to CISO / Chief Legal Counsel.
2. Dispatch compliance signal to team Slack channel (#sec-alerts).
